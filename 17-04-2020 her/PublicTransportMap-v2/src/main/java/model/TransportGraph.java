...
public class TransportGraph {
    ...
    public void addEdge(Connection connection) {
        Integer from = stationIndices.get(connection.getFrom().getStationName());
        Integer to = stationIndices.get(connection.getTo().getStationName());
    ...
    public void addWeights(String[] lineData, double[] weights) {
        // lineData example:
        //   {"purple", "metro", "Grote Sluis", "Grootzeil", "Coltrane Cirkel", "Centrum", "Swingstraat"};
        // weights example:
        //   {6.2, 5.2, 3.8, 3.6};

        // station names start from index 2..length
        for (int i = 2, weightsIndex = 0; i < weights.length; i++, weightsIndex++) {
            String fromStationString = lineData[i];
            String toStationString = lineData[i + 1];

            int indexOfFromStation = getIndexOfStationByName(fromStationString);
            int indexOfToStation = getIndexOfStationByName(toStationString);

            // get connection from 2D connections array
            // set weight on connection
            Connection fromToConnection = connections[indexOfFromStation][indexOfToStation];
            fromToConnection.setWeight(weights[weightsIndex]);
//            connections[indexOfFromStation][indexOfToStation] = fromToConnection;
            connections[indexOfFromStation][indexOfToStation] = new Connection(fromToConnection.getFrom(), fromToConnection.getTo(),
                    fromToConnection.getWeight(), fromToConnection.getLine());

            Connection toFromConnection = connections[indexOfToStation][indexOfFromStation];
            toFromConnection.setWeight(weights[weightsIndex]);
//            connections[indexOfToStation][indexOfFromStation] = toFromConnection;
            connections[indexOfToStation][indexOfFromStation] = new Connection(toFromConnection.getFrom(), toFromConnection.getTo(),
                    toFromConnection.getWeight(), toFromConnection.getLine());
        }
        System.out.println();
    }
    ...
    public static class Builder {
        ...
        public Builder addLine(String[] lineDefinition) {
            // map this: {"red", "metro", "A", "B", "C", "D"} -> Line object

            Line lineToAdd = new Line(lineDefinition[1], lineDefinition[0]);

            for (int i = 2; i < lineDefinition.length; i++) {
                Station station = new Station(lineDefinition[i]);

                lineToAdd.addStation(station);
            }
        ...
        public Builder buildStationSet() {
            for (Line l : lineList) {
                stationSet.addAll(l.getStationsOnLine());
            }
        ...
        public Builder buildConnections() {
            ...
            // use list of lines
            for (Line line : lineList) {
                // create connection for each consecutive station
                int amountOfStations = line.getStationsOnLine().size();
...
//                    Connection connection = new Connection(from, to, 1, line);
